...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        probs_grad = probs
        probs_grad[np.arange(N), y] -= 1

        db2 = np.sum(probs_grad, axis=0) / N
        dW2 = a1.T.dot(probs_grad) / N + 2 * reg * W2

        da1 = probs_grad.dot(W2.T)
        # set the gradient of inactive elem to be zero
        dh1 = da1
        dh1[a1 == 0] = 0

        db1 = np.sum(dh1, axis=0) / N
        dW1 = X.T.dot(dh1) / N + 2 * reg * W1
