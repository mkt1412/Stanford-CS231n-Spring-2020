...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # (2)
        embedded_caption_in, cache_embed_in = word_embedding_forward(captions_in, W_embed)
        # (3)
        forward_net = {'lstm': lstm_forward, 'rnn': rnn_forward}[self.cell_type]
        h, cache_rnn = forward_net(embedded_caption_in, h0, Wx, Wh, b)
        # (4)
    ...
    def sample(self, features, max_length=30):
        ...
        # use image features as h0
        h = features.dot(W_proj) + b_proj
        c = np.zeros(h.shape)
        V, W = W_embed.shape
        # feed <START> token into RNN
        x = np.ones((N, W)) * W_embed[self._start]

        # loop to generate whole caption
