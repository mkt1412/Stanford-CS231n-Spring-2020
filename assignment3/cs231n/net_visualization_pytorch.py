...
def compute_saliency_maps(X, y, model):
    ...
    # forward pass
    scores = model(X) # (N, C)
    scores = scores.gather(1, y.view(-1, 1)).squeeze() # subtract the socres with correct label
    ...
    # modify the gradient of X to output saliency map
    # abs -> max -> (H, W)
    m = abs(X.grad.data.cpu().numpy()) # convert to numpy array
    saliency = np.max(m, axis=1)
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(100):
        ...
        if pred == target_y:
            print("the classifier has been fooled")
            break
            
        # backprop target socres wrt image pixels
        ...
        # zero out the gradients on input images for next step
