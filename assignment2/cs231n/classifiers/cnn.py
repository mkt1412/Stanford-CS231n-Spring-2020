...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        # out.flatten() (num_filters, input_dim[1]/2, input_dim[2]/2)
        # compute the height and width after pooling using formula
        HP = (H - 2) // 2 + 1
        WP = (W - 2) // 2 + 1
        W2 = np.random.randn(num_filters*HP*WP, hidden_dim) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        # out = out.reshape(num_filters*HP*WP,)
        ...
        # add L2reg to wegith matrix
        for i in range(3):
          Wk = self.params['W' + str(i + 1)]
          loss += 0.5 * self.reg * np.sum(Wk * Wk) 

        # output layer
        dout, dW3, db3 = affine_backward(dout, out_cache)
